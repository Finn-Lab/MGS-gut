    ...
    if sys.argv[5]:
        if sys.argv[5] == "use_prefix":
            ...
            for n,i in enumerate(fields):
                if i == "metaspades":
                    store = n
            prefix = os.path.abspath(sys.argv[1]).split("/")[store-1]
        else:
            prefix = sys.argv[5]
except:
    prefix = ""
...
print "%s_%s\t%s\t%i\t%.2f\t%i\t%.2f\t%.2f\t%.2f" % (prefix, base, ref, lenref, float(aliref), lenquer, float(alique), float(ident), float(dist)) 
